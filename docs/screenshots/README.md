...
## 已补充的截图（README「界面预览与核心流程」已引用）

| 文件名 | 内容 | 状态 |
|--------|------|------|
| `dual-mode-entry.png` | 员工后台「预审单」入口 + 访客扫码自助预约页（`/public/appointment`） | ✅ 已截取 |
| `host-review.png` | 被访人（员工）在「我的预约 / 待审核」中审批访客 | ✅ 已截取 |
| `guard-checkin.png` | 门卫搜索访客、黑名单拦截、按类型自动匹配通行牌颜色并签到 | ✅ 已截取 |
