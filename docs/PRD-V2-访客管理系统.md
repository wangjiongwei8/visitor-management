...
| 项目代号 | visitor-management-v2 |
| 文档版本 | V2.0 |
| 创建日期 | 2025-07-10 |
| 基于版本 | V1.0（内部生产版本，不开源） |
| 发布模式 | 开源（GitHub） |

### 原始需求复述

在 v1 访客管理系统基础上进行重大升级，作为全新开源版本发布。核心变化：将"员工在客户内网帮访客创建预约"改为"访客自助扫码填表"，将"管理员统一审批"改为"被访人本人审核（可开关）"，同时完整保留 v1 全部已有功能。
...
| 编号 | 问题 | 影响范围 |
|------|------|----------|
| Q-01 | 访客编号（visitorCode）生成规则是否沿用 v1？还是需要调整为更语义化的格式？ | P0-06 门卫签到 |
...
| Q-05 | 固定二维码的 URL 路径是否为 `/appointment`（沿用 v1）？还是改为更短路径？ | P0-01 二维码 |
| Q-06 | v1 中员工可帮访客创建预约，v2 是否完全移除该功能？还是保留为可选项？ | P0-02 访客预约 |
...
以下 v1 模块在 v2 中完整保留，仅做必要的适配（如技术栈升级、API 路径调整）：
